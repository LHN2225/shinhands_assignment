...
--CAU 4--
select r.firstname, r.lastname, p.position, r.campusname, ROUND((p.yearlymembershipfee/ 12), 4) as Monthly_Dues
from (
        select m.firstname, m.lastname, m.positionid, c.campusname
        from members m
        join (  select campusid, campusname
                from campus
            ) c
        on m.campusid = c.campusid) r
join position p
on r.positionid = p.positionid
order by r.campusname desc, r.lastname asc;
